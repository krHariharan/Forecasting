...
# params_2_yrs = parameter_gen(dataframe.index.min()+datetime.timedelta(days=1), dataframe.index.max(), 730).reset_index(drop=True)
# annual_params = parameter_gen(dataframe.index.min()+datetime.timedelta(days=1), dataframe.index.max(), 365).reset_index(drop=True)
# quarterly_params = parameter_gen(dataframe.index.min()+datetime.timedelta(days=1), dataframe.index.max(), 121).reset_index(drop=True)
# monthly_params = parameter_gen(dataframe.index.min()+datetime.timedelta(days=1), dataframe.index.max(), 30).reset_index(drop=True)
# weekly_params = parameter_gen(dataframe.index.min()+datetime.timedelta(days=1), dataframe.index.max(), 7).reset_index(drop=True)
...
if datasetFull.shape[0] % 2 == 0:
    ...
    datasetFull = datasetFull[1:, :] - datasetFull[:-1, :]
print(datasetFull)
...
plt.savefig("vmd_lstm_delta.png")